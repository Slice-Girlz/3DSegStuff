...
and creates new dilated + closed masks under each ome.zarr file.
...
def dilate_masks(input_dir, dilation_steps=1):
    """
    Dilates + closes sparse_label_masks for all .ome.zarr files in input_dir.
    Writes results to labels/sparse_label_masks_dilated_{radius}/0, preserving
    the source's (1, Z, Y, X) shape, chunks, and dtype.

    For each Z-slice:
        1. Dilation with a disk of radius=dilation_steps (grows the mask).
        2. Closing with the same disk (fills small interior holes / gaps).
    """
    ...
    for f in zarr_files:
        ...
        dst_path = os.path.join(
            f, f"labels/sparse_label_masks_dilated_{dilation_steps}/0"
        )
        ...
        if squeezed.ndim == 2:
            d = morphology.dilation(squeezed, selem)
            processed = morphology.closing(d, selem)
        elif squeezed.ndim == 3:
            processed = np.empty_like(squeezed)
            for z in range(squeezed.shape[0]):
                d = morphology.dilation(squeezed[z], selem)
                processed[z] = morphology.closing(d, selem)
        else:
            raise ValueError(
                f"Unexpected squeezed shape {squeezed.shape} for {f}"
            )

        out = processed.reshape(original_shape).astype(src.dtype)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Dilate + close sparse_label_masks in OME-zarr files."
    )
    parser.add_argument(
        "--input_dir",
        type=str,
