...
Note: funlib stores world coordinates as *integers*, so all voxel sizes — both
extracted and the defaults — are expressed in nanometres (e.g. 0.2 um -> 200 nm).
Microns would round sub-micron sizes to zero, so the unit is fixed to "nm".
"""
...
DEFAULT_VOXEL_SIZE = (100, 100, 100)
DEFAULT_OFFSET = (0, 0, 0)
DEFAULT_UNITS = ("nm", "nm", "nm")
...
def _derive_funlib_from_metadata(native_metadata) -> dict:
    ...
    Returns a partial dict ({"voxel_size": ...}) when we recognise the metadata,
    or {} so `prepare_metadata` falls back to the defaults.
    """
    ...
    return {"voxel_size": voxel_size_zyx}


def _nd2_voxel_size_um(meta):
    """Nikon .nd2 metadata: per-axis calibration in microns, ordered (x, y, z)."""
    if not hasattr(meta, "channels"):  # not nd2 metadata -> let the next reader try
        return None
